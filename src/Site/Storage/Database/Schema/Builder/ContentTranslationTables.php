...
namespace Bundle\Site\Storage\Database\Schema\Builder;


use Bolt\Config;
use Bolt\Storage\Database\Schema\Builder\BaseBuilder;
...
class ContentTranslationTables extends BaseBuilder
{
    /**
     * @var array
     */
    ...
    public function setLocaleSlugs($slugs)
    {
        $this->localeSlugs = $slugs;
    }
    ...
    public function getSchemaTables(Schema $schema, Config $config)
    {
        ...
        foreach ($this->tables->keys() as $name) {
            ...
            $contentType = $contentTypes[$table->getContentType()];
            ...
            if (isset($contentType['fields']) && is_array($contentType['fields'])) {
                ...
                $this->addContentTypeTableColumns($this->tables[$name], $tables[$name], $fields, $fieldManager);
            }
        }
