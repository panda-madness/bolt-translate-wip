...
class LocalizedContentTables extends ContentTables {

    protected $localeSlugs;

    public function __construct(Connection $connection, Manager $manager, Pimple $tables, $charset, $collate, LoggerInterface $systemLog, FlashLoggerInterface $flashLogger, $localeSlugs = null)
    {
        parent::__construct($connection, $manager, $tables, $charset, $collate, $systemLog, $flashLogger);

        $this->localeSlugs = $localeSlugs;
    }
    ...
    public function getSchemaTables(Schema $schema, Config $config)
    {
        ...
        return $this->tableSchemas = array_merge($tables, $this->addLocalizedTables($schema, $fieldManager, $contentTypes));
    }

    private function addLocalizedTables($schema, $fieldManager, $contentTypes) {
        $tables = [];
        ...
        foreach ($this->tables->keys() as $name) {
            ...
            foreach ($this->localeSlugs as $locale) {
                $tableLocaleName = $name . '_' . $locale;
                $tables[$tableLocaleName] = $table->buildTable($schema, $tableLocaleName, $this->charset, $this->collate)
                    ->dropColumn('slug');


                if (isset($contentType['fields']) && is_array($contentType['fields'])) {
                    $fields = array_filter($contentType['fields'], function($options) { return isset($options['translatable']) && $options['translatable'] === true; });

                    $this->addContentTypeTableColumns($this->tables[$name], $tables[$tableLocaleName], $fields, $fieldManager);
                }

                /** @var Table $localizedTable */
                $localizedTable = $tables[$tableLocaleName];
                $localizedTable->addColumn('content_id', 'integer');
                $localizedTable->addForeignKeyConstraint($name, ['content_id'], ['id']);
            }
        }
